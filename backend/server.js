...
app.post('/api/admin/scheduler/ingest/:playCricketId', apiLimiter, async (req, res) => {
  ...
  const db = require("./db/schema").getDbAsync()
  const row = await db.prepare(`SELECT * FROM scheduled_fixtures WHERE play_cricket_id = ?`).get(playCricketId)
  ...
  const alreadyIngested = await db.prepare(`SELECT 1 FROM fixtures WHERE play_cricket_id = ? LIMIT 1`).get(String(playCricketId))
  if (alreadyIngested) {
    await db.prepare(`UPDATE scheduled_fixtures SET status='done', ingest_token=NULL WHERE play_cricket_id=?`).run(playCricketId)
  ...
  await db.prepare(`UPDATE scheduled_fixtures SET ingest_token = NULL, attempt_count = attempt_count + 1 WHERE play_cricket_id = ?`).run(playCricketId)
  ...
  try {
    ...
    await db.prepare(`UPDATE scheduled_fixtures SET status='done', ingested_at=?, cron_job_id=NULL WHERE play_cricket_id=?`)
    ...
    await db.prepare(`UPDATE scheduled_fixtures SET status=?, error_msg=? WHERE play_cricket_id=?`)

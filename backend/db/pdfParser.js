...
function parsePdfText(text) {
  ...
  for (const line of lines) {
    ...
    const tossMatch = line.match(/Toss\s*(.+?)\s+(?:won the toss and elected to|elected to)\s+(bat|field|bowl)/i);
    if (tossMatch) {
...
function extractInningsData(text, homeTeam, awayTeam) {
  ...
  if (homeStart !== -1 && awayStart !== -1) {
    if (homeStart < awayStart) {
      sections.push({ battingTeam: homeTeam, bowlingTeam: awayTeam, section: text.slice(homeStart, awayStart) });
      sections.push({ battingTeam: awayTeam, bowlingTeam: homeTeam, section: text.slice(awayStart) });
    } else {
      sections.push({ battingTeam: awayTeam, bowlingTeam: homeTeam, section: text.slice(awayStart, homeStart) });
...
function parseTeamSection(section, battingTeam, bowlingTeam, players) {
  ...
  if (battingStart !== -1) {
    const battingSection = bowlingStart !== -1
      ? section.slice(battingStart, bowlingStart)
      : section.slice(battingStart);
...
function parseBattingLine(line) {
  ...
  const body = t.replace(/\s+\d+\s+\d+\s+\d+\s+\d+\s*$/, '')

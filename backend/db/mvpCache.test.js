...
describe('mvp_cache fixture_id is TEXT', () => {
  ...
  it('accepts a manual (text) fixture_id without datatype mismatch', () => {
    ...
    const row = db.prepare('SELECT fixture_id FROM mvp_cache WHERE fixture_id = ?').get('manual-1779054809040')
  ...
  it('column type is TEXT', () => {
    const col = db.prepare('PRAGMA table_info(mvp_cache)').all().find(c => c.name === 'fixture_id')

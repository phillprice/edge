...
function mergeSyntheticPlayer(db, realId, name) {
  const row = db
    .prepare('SELECT player_id FROM players WHERE player_id < 0 AND LOWER(name) = LOWER(?)')
    .get(name)
  if (!row) return
  ...
  db.transaction(() => {
    ...
    const pf = db
      .prepare('SELECT fixture_id, is_captain, is_wk FROM player_flags WHERE player_id = ?')
      .all(synthId)
    for (const f of pf) {
      db.prepare(
        `
    ...
    const mc = db
      .prepare('SELECT fixture_id, innings_order FROM match_captains WHERE player_id = ?')
      .all(synthId)
    for (const c of mc) {
      db.prepare(
        'INSERT OR REPLACE INTO match_captains (fixture_id, innings_order, player_id) VALUES (?, ?, ?)'
      ).run(c.fixture_id, c.innings_order, realId)
    }
    ...
    const wk = db
      .prepare(
        'SELECT fixture_id, innings_order, from_over, to_over FROM wk_assignments WHERE player_id = ?'
      )
      .all(synthId)
    for (const w of wk) {
      db.prepare(
        'INSERT OR IGNORE INTO wk_assignments (fixture_id, innings_order, player_id, from_over, to_over) VALUES (?, ?, ?, ?, ?)'
      ).run(w.fixture_id, w.innings_order, realId, w.from_over, w.to_over)
    }
...
function ingestDeliveries(fixtureId, inningsOrder, resultId, inningsJson, matchMeta) {
  ...
  const lookupByName = db.prepare(
    'SELECT player_id FROM players WHERE LOWER(name) = LOWER(?) LIMIT 1'
  )
  const synthPlayers = []
  ...
  if (matchMeta?.innings) {
    ...
    if (inningsData?.batters?.length) {
      ...
      db.prepare('DELETE FROM dismissals WHERE fixture_id = ? AND innings_order = ?').run(
        fixtureId,
        inningsOrder
      )
...
function autoPopulateRoles(fixtureId) {
  ...
  db.transaction(() => {
    ...
    if (oppBattingOrder !== null) {
      db.prepare('DELETE FROM wk_assignments WHERE fixture_id = ? AND innings_order = ?').run(
        fixtureId,
        oppBattingOrder
      )
    }

...
router.get('/', async (req, res) => {
  ...
  const fixtures = await db.prepare(`${FIXTURE_SELECT} LIMIT ? OFFSET ?`).all(...accessParams, limit, offset);
...
router.get('/season', async (req, res) => {
  ...
  const bowlRow = await db.prepare(`
  ...
  const topBatterRows = await db.prepare(`
  ...
  const topBowlerRows = await db.prepare(`
    SELECT p.player_id, p.name,
  ...
  const matchScoreFixtures = await db.prepare(`
...
router.get('/:fixtureId', async (req, res) => {
  ...
  let mvp, mvpMeta;
  if (isManualMatch) {
    const cachedMvp = await db.prepare('SELECT players_json FROM mvp_cache WHERE fixture_id = ?').get(fixtureId);
    if (cachedMvp) {
      mvp = JSON.parse(cachedMvp.players_json);
    } else {
      mvp = buildManualMvp(db, fixtureId);
      if (mvp.length) {
        await db.prepare('INSERT OR REPLACE INTO mvp_cache (fixture_id, players_json, meta_json, computed_at) VALUES (?, ?, ?, ?)')
    ...
    const cached = await db.prepare('SELECT players_json, meta_json FROM mvp_cache WHERE fixture_id = ?').get(fixtureId);
    if (cached) {
  ...
  let partnerships = [];
  if (hasDeliveries) {
    const detailCache = await db.prepare('SELECT partnerships_json, phases_json FROM match_detail_cache WHERE fixture_id = ?').get(fixtureId);
    if (detailCache) {
...
router.get('/:fixtureId/roles', async (req, res) => {
  const db = getDbAsync();
  ...
  const fixtureTeams = await db.prepare('SELECT home_team, away_team FROM fixtures WHERE fixture_id = ?').get(fixtureId);
  ...
  const isManualFixture = !!(
    await db.prepare(`SELECT 1 FROM manual_batting WHERE fixture_id = ? LIMIT 1`).get(fixtureId) ||
    await db.prepare(`SELECT 1 FROM manual_bowling WHERE fixture_id = ? LIMIT 1`).get(fixtureId)
  );
  ...
  for (const inn of inningsList) {
    ...
    if (isManualFixture) {
      ...
      players = order === 1
        ? await db.prepare(`SELECT DISTINCT p.player_id, p.name FROM players p JOIN manual_batting mb ON mb.player_id = p.player_id WHERE mb.fixture_id = ? ORDER BY p.name`).all(fixtureId)
        : await db.prepare(`SELECT DISTINCT p.player_id, p.name FROM players p JOIN manual_bowling mbw ON mbw.player_id = p.player_id WHERE mbw.fixture_id = ? ORDER BY p.name`).all(fixtureId);
    } else {
    ...
    const allByes = await db.prepare(
...
router.put('/:fixtureId/captain', async (req, res) => {
  const db = getDbAsync();
  const { fixtureId } = req.params;
...
router.post('/:fixtureId/wk', async (req, res) => {
  const db = getDbAsync();
  const { fixtureId } = req.params;
  ...
  for (const e of existing) {
    ...
    if (eTo === null) {
      await db.prepare('UPDATE wk_assignments SET to_over = ? WHERE id = ?').run(from_over - 1, e.id);
    } else {
...
router.patch('/:fixtureId/wk/:wkId', async (req, res) => {
  const db = getDbAsync();
  const { fixtureId, wkId } = req.params;
  const { to_over } = req.body;
  const stint = await db.prepare('SELECT * FROM wk_assignments WHERE id = ? AND fixture_id = ?').get(wkId, fixtureId);
  ...
  await db.prepare('UPDATE wk_assignments SET to_over = ? WHERE id = ?').run(to_over ?? null, wkId);
  res.json({ ok: true });
});

// DELETE /api/matches/:fixtureId/wk/:wkId
router.delete('/:fixtureId/wk/:wkId', async (req, res) => {
  const db = getDbAsync();
  await db.prepare(`DELETE FROM wk_assignments WHERE id = ? AND fixture_id = ?`)
    .run(req.params.wkId, req.params.fixtureId);
  res.json({ ok: true });
});

// POST /api/matches/:fixtureId/wk-error  { innings_order, player_id, error_type }
router.post('/:fixtureId/wk-error', async (req, res) => {
  const db = getDbAsync();
  const { fixtureId } = req.params;
...
router.delete('/:fixtureId/wk-error/:errorId', async (req, res) => {
  const db = getDbAsync();
  await db.prepare(`DELETE FROM wk_errors WHERE id = ? AND fixture_id = ?`)
...
router.patch('/:fixtureId/delivery/:deliveryId', async (req, res) => {
  const db = getDbAsync();
  ...
  await db.transaction(async (db) => {
    ...
    if (sets.length) {
      await db.prepare(`UPDATE deliveries SET ${sets.join(', ')} WHERE id = ?`).run(...vals, deliveryId);
    }
...
router.patch('/:fixtureId/pair-block', async (req, res) => {
  const db = getDbAsync();
  const { fixtureId } = req.params;
  ...
  const inn = await db.prepare(`SELECT result_id FROM innings WHERE fixture_id = ? AND innings_order = ?`).get(fixtureId, innings_order);
...
router.patch('/:fixtureId/result', async (req, res) => {
  const db = getDbAsync();
  const { fixtureId } = req.params;
  const fixture = await db.prepare('SELECT fixture_id FROM fixtures WHERE fixture_id = ?').get(fixtureId);
...
router.post('/:fixtureId/innings', async (req, res) => {
  const db = getDbAsync();
  const { fixtureId } = req.params;
  ...
  let row = await db.prepare('SELECT result_id, innings_order FROM innings WHERE fixture_id = ? AND innings_order = ?').get(fixtureId, order);
  let created = false;
  if (!row) {
    const r = await db.prepare('INSERT INTO innings (fixture_id, innings_order) VALUES (?, ?)').run(fixtureId, order);
...
//          dismissed_batter_id?, dismissal_method?, dismissal_fielder_id?, dismissal_bowler_id? }
router.post('/:fixtureId/innings/:inningsOrder/delivery', async (req, res) => {
  const db = getDbAsync();
  ...
  const inn = await db.prepare('SELECT result_id FROM innings WHERE fixture_id = ? AND innings_order = ?').get(fixtureId, order);
  ...
  await db.transaction(async (db) => {
    ...
    if (!last) {
      ...
      const legalInOver = await db.prepare(
...
router.delete('/:fixtureId/delivery/:deliveryId', async (req, res) => {
  const db = getDbAsync();
  const { fixtureId, deliveryId } = req.params;
  if (!fixtureId.startsWith('manual-')) return res.status(403).json({ error: 'Only allowed on manual fixtures' });
  ...
  await db.transaction(async (db) => {
    ...
    await db.prepare('DELETE FROM deliveries WHERE id = ?').run(deliveryId);
  })();

...
router.get('/prefs', (req, res) => {
  ...
  const rows = db
    .prepare(`SELECT notif_type, channel, enabled FROM notification_prefs WHERE clerk_user_id = ?`)
    .all(uid)
  const prefs = {}
  for (const r of rows) {
...
router.post('/player-follows', (req, res) => {
  ...
  const player = db
    ...
    .get(Number(player_id))
  if (!player) return res.status(404).json({ error: 'Player not found' })

  db.prepare(
    `INSERT OR IGNORE INTO player_follows (clerk_user_id, player_id, channel) VALUES (?, ?, ?)`
  ).run(uid, Number(player_id), channel)
  res.json({ ok: true })
})
...
router.delete('/player-follows/:playerId', (req, res) => {
  const uid = userId(req)
  const db = getDb()
  db.prepare(`DELETE FROM player_follows WHERE clerk_user_id = ? AND player_id = ?`).run(
    uid,
...
router.get('/telegram', (req, res) => {
  const uid = userId(req)
  const row = getDb()
    .prepare(`SELECT chat_id, registered_at FROM user_telegram WHERE clerk_user_id = ?`)
    .get(uid)
...
router.put('/telegram', (req, res) => {
  ...
  if (!chat_id || !/^\d+$/.test(String(chat_id)))
    return res.status(400).json({ error: 'chat_id must be a numeric string' })
...
async function unsubscribeHandler(req, res) {
  ...
  if (!token)
    return res.status(400).send('<html><body><p>Invalid unsubscribe link.</p></body></html>')

  const db = getDb()
  const row = db
    .prepare(`SELECT clerk_user_id, notif_type FROM notification_prefs WHERE unsub_token = ?`)
    .get(token)
  if (!row)
    return res
      .status(404)
      .send('<html><body><p>Unsubscribe link not found or already used.</p></body></html>')
  ...
  const html =
    ...
    '<h2>Unsubscribed</h2>' +
    "<p>You've been unsubscribed from <strong>" +
    label +
    '</strong>.</p>' +
    '<p><a href="' +
    appUrl +
    '/notifications">Manage all preferences</a></p>' +
    '</body></html>'
  res.send(html)
}

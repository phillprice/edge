...
// GET /api/access-requests/my-groups
router.get('/my-groups', async (req, res) => {
  ...
  let rows
  if (isSuperAdmin) {
    rows = await db.prepare(`SELECT team_id, season_id, label, year FROM watched_teams ORDER BY year DESC, label ASC`).all()
  } else {
    ...
    rows = await db.prepare(`SELECT wt.team_id, wt.season_id, wt.label, wt.year FROM watched_teams wt WHERE ${clauses} ORDER BY wt.year DESC, wt.label ASC`).all(...params)
  }
  res.json(rows.map(r => ({
...
router.get('/count', async (req, res) => {
  ...
  if (isSuperAdmin || groups.length === 0) {
    const row = await db.prepare(`SELECT COUNT(*) AS count FROM access_requests WHERE status = 'pending'`).get()
  ...
  const row = await db.prepare(`SELECT COUNT(*) AS count FROM access_requests WHERE status = 'pending' AND (${placeholders})`).get(...params)
...
router.patch('/:id', async (req, res) => {
  ...
  const db = getDbAsync()
  const request = await db.prepare('SELECT * FROM access_requests WHERE id = ?').get(req.params.id)
  ...
  await db.prepare(`UPDATE access_requests SET status = ?, resolved_by = ?, resolved_at = datetime('now') WHERE id = ?`)

  ...
  return {
    ...
    htmlContent: wrap(
      '<p>Hi,</p>' +
        ...
        '</strong> (' +
        eEmail +
        ') has requested access to <strong>' +
        eTeam +
        '</strong>.</p>' +
        '<p><a href="' +
        escHtml(adminUrl) +
  ...
  return {
    ...
    htmlContent: wrap(
      '<p>Hi ' +
        eName +
        ',</p>' +
        '<p>Your request to access <strong>' +
        eTeam +
        ...
        (approved
          ? '<p><a href="' +
            escHtml(appUrl) +
            '" style="background:#690028;color:#fff;padding:8px 16px;text-decoration:none;border-radius:4px;display:inline-block">View match stats</a></p>'
  ...
  const statsLines = [
    ...
    topBowl
      ? statRow(
          'Top bowl',
          '<strong>' +
            escHtml(topBowl.name) +
            '</strong> ' +
            escHtml(topBowl.wickets) +
            '/' +
            escHtml(topBowl.runs)
        )
      : '',
    mvp
  ...
  return {
    ...
    htmlContent: wrap(
      '<p>Hi ' +
        ...
        resultLine +
        (statsLines
          ? '<table style="border-collapse:collapse;margin:16px 0">' + statsLines + '</table>'
          : '') +
        '<p><a href="' +
        escHtml(matchUrl) +
        '" style="background:#690028;color:#fff;padding:8px 16px;text-decoration:none;border-radius:4px;display:inline-block">View full scorecard</a></p>' +
        '<p style="font-size:12px;color:#888;margin-top:32px"><a href="' +
        escHtml(unsubLink) +
  ...
  return {
    ...
    htmlContent: wrap(
      '<p>Hi ' +
        ...
        '<p><strong>' +
        escHtml(playerName) +
        '</strong> hit a milestone:</p>' +
        '<ul style="padding-left:20px">' +
        items +
        '</ul>' +
        '<p><a href="' +
        escHtml(matchUrl) +
        '" style="background:#690028;color:#fff;padding:8px 16px;text-decoration:none;border-radius:4px;display:inline-block">View match</a></p>' +
        '<p style="font-size:12px;color:#888;margin-top:32px"><a href="' +
        escHtml(unsubLink) +
  ...
  const rows = requests
    .map(
      (r) =>
        ...
        escHtml(r.user_name || r.user_email || r.clerk_user_id) +
        '</td><td style="padding:4px 8px">' +
        escHtml(r.team_label || 'team ' + r.team_id) +
        '</td><td style="padding:4px 8px;color:#888">' +
        escHtml(r.requested_at?.slice(0, 10)) +
        '</td></tr>'
    )
    .join('')
  return {
    subject:
      requests.length + ' access request' + (requests.length === 1 ? '' : 's') + ' pending action',
    htmlContent: wrap(

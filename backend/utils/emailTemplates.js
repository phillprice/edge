...
function tmplNewMatch(opts) {
  ...
  const statsLines = [
    topBat  ? statRow('Top bat', '<strong>' + escHtml(topBat.name) + '</strong> ' + escHtml(topBat.runs) + ' (' + escHtml(topBat.balls) + 'b)') : '',
    topBowl ? statRow('Top bowl', '<strong>' + escHtml(topBowl.name) + '</strong> ' + escHtml(topBowl.wickets) + '/' + escHtml(topBowl.runs)) : '',
    mvp     ? statRow('MVP', '<strong>' + escHtml(mvp.name) + '</strong> (' + escHtml(mvp.pts) + ' pts)') : ''
  ].filter(Boolean).join('')
  return {
...
function tmplPendingRequestsDigest({ requests }) {
  const rows = requests.map(r =>
    '<tr><td style="padding:4px 8px">' + escHtml(r.user_name || r.user_email || r.clerk_user_id) +
    '</td><td style="padding:4px 8px">' + escHtml(r.team_label || 'team ' + r.team_id) +
    '</td><td style="padding:4px 8px;color:#888">' + escHtml(r.requested_at?.slice(0, 10)) + '</td></tr>'
  ).join('')
  return {

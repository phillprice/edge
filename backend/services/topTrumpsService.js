...
function computeTopTrumps(db, req) {
  ...
  const gcRows = db
    ...
    whcc_players AS (
      SELECT player_id FROM players WHERE ${clubFilters.playerWhere('players')}
    ),
    bat_pts AS (
  ...
  const gcMap = new Map(gcRows.map((r) => [r.player_id, r.gamechanger_count]))
  ...
  return allStats
    ...
    .map((r) => {
      ...
      const gcBonus = Math.min(10, gamechanger)
      const overall = clamp100(0.45 * batting + 0.3 * bowling + 0.15 * fielding + gcBonus)

...
function FetchPanel() {
  ...
  return (
    <div className="card" style={{ marginBottom: '1.5rem' }}>
      ...
      {error && <div className="alert alert-error" style={{ marginTop: '0.75rem' }}><strong>Error:</strong> {error}</div>}
    </div>
  )
}
...
function UploadPanel() {
  ...
  function handleFiles(incoming) {
    ...
    setResult(null); setError(null); setDuplicate(null)
  }
  ...
  async function submit(overwrite = false) {
    ...
    try {
      ...
      if (data.alreadyExists) { setDuplicate(data); return }
  ...
  return (
    <div className="card">
      <p style={{ marginBottom: '1rem', color: 'var(--text2)', fontSize: '0.9rem' }}>
        Upload the <strong>print.html scorecard</strong> and one or more <strong>innings JSON files</strong>.
        Re-uploading the same match is safe — it updates without creating duplicates.
      </p>
      <div
      ...
      >
        <input ref={inputRef} type="file" multiple accept=".html,.json"
      ...
      {files.length > 0 && (
        <div style={{ marginTop: '1rem' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {htmls.map(f => <FileRow key={f.name} file={f} type="HTML" onRemove={removeFile} />)}
            {jsons.map(f => <FileRow key={f.name} file={f} type="JSON" onRemove={removeFile} />)}
          </div>
          {jsons.length === 0 && (
        ...
        <div className="alert alert-success" style={{ marginTop: '1rem' }}>
          ...
          {result.matchMeta && (
            <div style={{ marginTop: '6px', fontSize: '0.85rem' }}>
              Match: {result.matchMeta.homeTeam} vs {result.matchMeta.awayTeam} — {result.matchMeta.matchDate}
            </div>
          )}
      ...
      {error && <div className="alert alert-error" style={{ marginTop: '1rem' }}><strong>Error:</strong> {error}</div>}
    </div>
  )
}
...
function ManualTab() {
  ...
  useEffect(() => {
    apiFetch('/api/admin/manual-matches').then(r => r.json()).then(setMatches).catch(() => setMatches([]))
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        <p style={{ color: 'var(--text2)', fontSize: '0.88rem' }}>Manually-entered match scorecards.</p>
            ...
            <tbody>
              {matches.map(m => (
                <tr key={m.fixture_id} style={{ borderTop: '1px solid var(--border)' }}>
                  ...
                  <td style={{ padding: '7px 12px', color: 'var(--text2)' }}>{m.competition || '—'}</td>
                  ...
                  <td style={{ padding: '7px 12px', display: 'flex', gap: 6 }}>
                    <button className="secondary" style={{ fontSize: '0.78rem', padding: '2px 10px' }}
                      onClick={() => navigate(`/match/${m.fixture_id}`)}>View</button>
                    <button className="secondary" style={{ fontSize: '0.78rem', padding: '2px 10px' }}
                      onClick={() => navigate(`/manual/${m.fixture_id}`)}>Edit</button>
                  </td>
                </tr>
              ))}
            </tbody>
...
function FileRow({ file, type, onRemove }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.88rem' }}>
      ...
      <button className="secondary" style={{ padding: '2px 8px', fontSize: '0.8rem' }}
        onClick={() => onRemove(file.name)}><X size={12} /></button>
    </div>
  )
}
    ...
    <div style={{ flex: 1, minWidth: 220 }}>
      ...
      {selectedPlayer ? (
        ...
        <div style={{ position: 'relative' }}>
          <input type="text" placeholder="Search name or ID…" value={query}
...
function AutoIngestPanel() {
  ...
  function loadStatus() {
    apiFetch('/api/admin/scheduler/status').then(r => r.json()).then(setStatus).catch(() => {})
  }
  ...
  async function run(endpoint) {
    ...
    try {
      ...
      loadStatus()
    } catch { setRunMsg({ ok: false, text: 'Failed' }) }
    setRunning(false)
  }
  ...
  async function rescan() {
    ...
    try {
      ...
      loadStatus()
    } catch (e) { setRescanMsg({ ok: false, text: e.message }) }
    setRescanning(false)
  }
    ...
    <div className="card" style={{ marginBottom: '1.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8, marginBottom: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <button className="secondary" style={{ fontSize: '0.82rem' }} disabled={running} onClick={() => run('discover')}>
      ...
      {runMsg    && <p style={{ fontSize: '0.82rem', color: runMsg.ok    ? 'var(--green)' : 'var(--red)', marginBottom: '0.5rem' }}>{runMsg.text}</p>}
      {rescanMsg && <p style={{ fontSize: '0.82rem', color: rescanMsg.ok ? 'var(--green)' : 'var(--red)', marginBottom: '0.5rem' }}>{rescanMsg.text}</p>}

      <form onSubmit={addTeam} style={{ display: 'flex', gap: '8px', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        <input
          type="url"
          ...
          style={{ flex: 1, minWidth: '280px' }}
        />
        <button type="submit" disabled={adding || !urlInput.trim()} style={{ whiteSpace: 'nowrap' }}>
      ...
      {status && status.teams.length > 0 && (() => {
        ...
        const sorted = [...visibleSeasons].sort((a, b) => {
          ...
          if (sortCol === 'pending') return sortDir === 'asc' ? (a.pending || 0) - (b.pending || 0) : (b.pending || 0) - (a.pending || 0)
          if (sortCol === 'done') return sortDir === 'asc' ? (a.done || 0) - (b.done || 0) : (b.done || 0) - (a.done || 0)
          return 0
        })
              ...
              <table style={{ fontSize: '0.8rem', width: '100%' }}>
                <thead>
                  <tr>
                    ...
                    <SortTh col="date"    label="Last match" sortCol={sortCol} sortDir={sortDir} onSort={onSort} style={{ textAlign: 'left', padding: '6px 10px' }} />
                    <SortTh col="pending" label="Pending"    sortCol={sortCol} sortDir={sortDir} onSort={onSort} style={{ textAlign: 'right', padding: '6px 10px' }} />
                    <SortTh col="done"    label="Done"       sortCol={sortCol} sortDir={sortDir} onSort={onSort} style={{ textAlign: 'right', padding: '6px 10px' }} />
                  ...
                  {sorted.map(t => (
                    <tr key={`${t.team_id}:${t.season_id}`} style={{ borderTop: '1px solid var(--border)' }}>
                      ...
                      <td style={{ padding: '5px 10px' }}>
                        <button className="secondary" style={{ fontSize: '0.72rem', padding: '1px 7px', color: 'var(--red)', borderColor: 'var(--red)' }}
                          onClick={() => { if (window.confirm(`Remove ${t.label} ${t.year}?`)) removeTeam(t.id) }}>
                          Remove
                        </button>
                      </td>
                    </tr>
      ...
      {status && status.teams.length === 0 && (
        <span style={{ fontSize: '0.82rem', color: 'var(--text3)' }}>No teams in system yet — add via the URL field above.</span>
      )}
    </div>
  )
}
...
function StaleFixturesPanel() {
  ...
  useEffect(() => {
    apiFetch('/api/admin/scheduler/stale').then(r => r.json()).then(d => setRows(Array.isArray(d) ? d : []))
      .catch(() => setRows([]))
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])
  ...
  async function ignoreSelected() {
    ...
    try {
      ...
      setSel(new Set())
      setMsg({ error: false, text: `${data.ignored} fixture${data.ignored === 1 ? '' : 's'} ignored.` })
    } catch (e) {
  ...
  function toggleAll() {
    setSel(s => s.size === rows.length ? new Set() : new Set(rows.map(r => r.play_cricket_id)))
  }
  ...
  return (
    <div className="card" style={{ marginTop: '1.5rem' }}>
      ...
      <p style={{ fontSize: '0.88rem', color: 'var(--text2)', marginBottom: '1rem' }}>
        Pending fixtures older than 7 days, and fixtures that failed all retries.
        Mark as ignored to stop them appearing in the queue.
      </p>
      <div style={{ display: 'flex', gap: 8, marginBottom: '0.75rem', alignItems: 'center' }}>
        ...
        <button disabled={!sel.size || saving} onClick={ignoreSelected} style={{ fontSize: '0.78rem', padding: '2px 10px' }}>
          {saving ? 'Saving…' : `Ignore ${sel.size || ''} selected`}
        </button>
      </div>
      {msg && <p style={{ fontSize: '0.82rem', color: msg.error ? 'var(--red)' : 'var(--green)', marginBottom: '0.5rem' }}>{msg.text}</p>}
  ...
  return (
    <div className="card" style={{ marginBottom: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
        {players.map(p => (
          <div key={p.player_id} style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', fontSize: '0.85rem' }}>
            <span style={{ color: 'var(--text3)', minWidth: 60 }}>#{p.player_id}</span>
            <span style={{ color: 'var(--text2)', minWidth: 120 }}>{p.name || '(no name)'}</span>
            <span style={{ color: 'var(--text3)', fontSize: '0.78rem', flex: 1 }}>{p.fixture_count} match{p.fixture_count !== 1 ? 'es' : ''}</span>
            <input
              type="text"
            ...
            />
            <button disabled={!names[p.player_id]?.trim() || saving[p.player_id]}
              onClick={() => saveName(p.player_id)}
              style={{ fontSize: '0.78rem', padding: '2px 10px' }}>
              {saving[p.player_id] ? '…' : saved[p.player_id] ? '✓' : 'Save'}
            </button>
            <button className="secondary" disabled={ignoring[p.player_id]}
          ...
          return (
            <div key={fid} style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', fontSize: '0.85rem' }}>
              ...
              {saved[fid] ? (
                ...
                <>
                  <select value={sel[fid] || ''} onChange={e => setSel(s => ({ ...s, [fid]: e.target.value }))} style={{ fontSize: '0.8rem' }}>
                    <option value="">— select team/season —</option>
                    {teams.map(t => (
                      <option key={`${t.team_id}:${t.season_id}`} value={`${t.team_id}:${t.season_id}`}>{t.label} {t.year}</option>
                    ))}
                  </select>
                  <button disabled={!sel[fid] || saving[fid]} onClick={() => associate(fid)} style={{ fontSize: '0.78rem', padding: '2px 10px' }}>
...
function MissingRolesPanel() {
  ...
  useEffect(() => {
    apiFetch('/api/admin/matches-missing-roles').then(r => r.json()).then(setMatches).catch(() => setMatches([]))
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem' }}>
        {matches.map(m => (
          <div key={m.fixture_id} style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', fontSize: '0.85rem' }}>
            <a href={`/match/${m.fixture_id}`} style={{ color: 'var(--accent)', fontWeight: 500, minWidth: 80 }}>#{m.fixture_id}</a>
            <span style={{ color: 'var(--text2)', flex: 1 }}>{shortTeam(m.home_team)} vs {shortTeam(m.away_team)}</span>
...
function MergePanel() {
  ...
  useEffect(() => {
    apiFetch('/api/players').then(r => r.json()).then(setPlayers).catch(() => {})
    ...
    <div className="card">
      ...
      {suggestions.length > 0 && (
        <div style={{ marginBottom: '1rem' }}>
          <div style={{ fontSize: '0.78rem', color: 'var(--text3)', marginBottom: 6 }}>Suggested duplicates</div>
          {suggestions.map(g => (
            <div key={g.name} style={{ marginBottom: 8 }}>
              <div style={{ fontSize: '0.82rem', fontWeight: 600, marginBottom: 4 }}>{g.name}</div>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {g.players.map(p => (
                  <button key={p.player_id} className="secondary" style={{ fontSize: '0.75rem', padding: '2px 8px' }}
                    onClick={() => { if (!keepId) setKeepId(p.player_id); else setDropId(p.player_id) }}>
      ...
      <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
        <PlayerSearch label="Keep (target)" players={players} selected={keepId} onSelect={setKeepId} exclude={dropId} />
        <PlayerSearch label="Drop (source)" players={players} selected={dropId} onSelect={setDropId} exclude={keepId} />
      </div>

      {msg && <p style={{ fontSize: '0.85rem', color: msg.error ? 'var(--red)' : 'var(--green)', marginBottom: '0.5rem' }}>{msg.text}</p>}

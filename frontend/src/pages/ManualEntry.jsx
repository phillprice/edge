...
const seasonFields = (key) =>
  key ? { team_id: Number(key.split(':')[0]), season_id: Number(key.split(':')[1]) } : {}
...
export default function ManualEntry() {
  ...
  async function selectFixture(id) {
    ...
    setEntrySeason(
      data.association ? `${data.association.team_id}:${data.association.season_id}` : ''
    )
    setBatting(
      data.batting.length
        ? data.batting.map((r) => ({
    ...
    setBowling(
      data.bowling.length
        ? data.bowling.map((r) => ({
            ...
            maidens: r.maidens,
            wicket_maidens: r.wicket_maidens,
            runs: r.runs,
            wickets: r.wickets,
            wides: r.wides,
            no_balls: r.no_balls
  ...
  function fixtureStatus(f) {
    ...
    if (!hasBatting && !hasBowling)
      return { label: 'Not started', color: 'var(--text3)', hasBatting, hasBowling }
    if (hasBatting && hasBowling)
      return { label: 'Complete ✓', color: 'var(--green)', hasBatting, hasBowling }
    return { label: 'Partial', color: 'var(--orange)', hasBatting, hasBowling }
  }

  const calcWhccOvers = (() => {
    const balls = batting
      .filter((r) => !r.did_not_bat && r.player_name.trim())
      .reduce((s, r) => s + (parseInt(r.balls) || 0), 0)
          ...
          {fixtures.length === 0 && (
            ...
            >
              <p style={{ color: 'var(--text2)', marginBottom: '0.75rem' }}>
                No matches available for manual entry
              </p>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      <span style={{ fontSize: '0.8rem', color: 'var(--text3)' }}>
                        {formatDateShort(f.match_date) || f.match_date}
                      </span>
                      {f.format === 'pairs' && (
                        <span
                          className="tag"
                      ...
                      {hasManual && (
                        <span
                          style={{ fontSize: '0.72rem', color: 'var(--text3)', marginLeft: '4px' }}
                        >
          ...
          <div className="form-grid">
            <label className="full">
              ...
              <input
                ...
                onChange={(e) => mf('date', e.target.value)}
              />
            </label>
            <label>
            ...
            {teams.length > 0 && (
              <label>
                ...
                <select
                  value={matchForm.team_season}
                  onChange={(e) => mf('team_season', e.target.value)}
                >
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) => mf('opponent', e.target.value)}
                placeholder="Opposition CC"
              />
            </label>
            <label>
              <span className="form-label">Home / Away</span>
              <select
                value={matchForm.is_home ? 'home' : 'away'}
                onChange={(e) => mf('is_home', e.target.value === 'home')}
              >
            ...
            <label>
              ...
              <input
                value={matchForm.ground}
                onChange={(e) => mf('ground', e.target.value)}
      ...
      {fixtureId && selectedFixture && (
        <>
            ...
            <div style={{ flex: 1 }}>
              ...
              >
                <span style={{ fontSize: '0.82rem', color: 'var(--text3)' }}>
                  {formatDateShort(selectedFixture.match_date) || selectedFixture.match_date}
                </span>
                {selectedFixture.format === 'pairs' && (
                  <span
                    className="tag"
                    style={{ background: 'var(--blue-bg)', color: 'var(--blue)' }}
                  >
              ...
              {teams.length > 0 && (
                ...
                >
                  ...
                  <span style={{ fontSize: '0.74rem', color: 'var(--text3)' }}>
                    (saved with stats)
                  </span>
                </label>
              )}
          ...
          <div className="card">
            <div className="tabs">
              <button
                className={`tab${tab === 'batting' ? ' active' : ''}`}
                onClick={() => setTab('batting')}
              >
                Batting
              </button>
              <button
                className={`tab${tab === 'bowling' ? ' active' : ''}`}
                onClick={() => setTab('bowling')}
              >
                Bowling
              </button>
              <button
                className={`tab${tab === 'fielding' ? ' active' : ''}`}
            ...
            {tab === 'batting' && (
              <>
                <BattingTable
                  rows={batting}
                  onChange={(i, f, v) =>
                    setBatting((rows) => rows.map((r, idx) => (idx === i ? { ...r, [f]: v } : r)))
                  }
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    {calcWhccOvers && (
                      <span style={{ fontSize: '0.8rem', color: 'var(--text3)' }}>
                        calculated: {calcWhccOvers} — override if needed
                      </span>
                    )}
                  </div>
                </div>
            ...
            {tab === 'bowling' && (
              <>
                <BowlingTable
                  rows={bowling}
                  onChange={(i, f, v) =>
                    setBowling((rows) => rows.map((r, idx) => (idx === i ? { ...r, [f]: v } : r)))
                  }
                  ...
                  >
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', margin: 0 }}>
                      ...
                      <input
                        ...
                        min="0"
                        value={bowlByes}
                        onChange={(e) => setBowlByes(e.target.value)}
                        style={{ width: '72px' }}
                      />
                    </label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', margin: 0 }}>
                      ...
                      <input
                        ...
                        min="0"
                        value={bowlLb}
                        onChange={(e) => setBowlLb(e.target.value)}
                        style={{ width: '72px' }}
                      />
                    </label>
                    <span style={{ fontSize: '0.8rem', color: 'var(--text3)' }}>
                      not credited to any bowler
                    </span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', margin: 0 }}>
                      ...
                      <input
                        ...
                        placeholder={calcOppOvers ?? 'e.g. 20.0'}
                        style={{ width: '90px' }}
                      />
                    </label>
                    {calcOppOvers && (
                      <span style={{ fontSize: '0.8rem', color: 'var(--text3)' }}>
                        calculated: {calcOppOvers} — override if needed
                      </span>
                    )}
                  </div>
                </div>
            ...
            {tab === 'fielding' && (
              <FieldingTable
                rows={fielding}
                onChange={(i, f, v) =>
                  setFielding((rows) => rows.map((r, idx) => (idx === i ? { ...r, [f]: v } : r)))
                }
              ...
              <div className="form-grid">
                <label style={{ display: 'block' }}>
                  ...
                  <select value={captainName} onChange={(e) => setCaptainName(e.target.value)}>
                    ...
                    {batting
                      .filter((r) => r.player_name.trim() && !r.did_not_bat)
                      .map((r) => (
                        <option key={r.player_name} value={r.player_name}>
                          {r.player_name}
                        </option>
                      ))}
                  </select>

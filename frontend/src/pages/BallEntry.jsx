    ...
    try {
      ...
      if (!isWide) {
        const totalRuns =
          extrasType === null
            ? Number(runsBat)
            : Number(runsExtra) + (extrasType === 1 ? Number(runsBat) : 0)
        const isOdd = totalRuns % 2 === 1

        // End of legal over — swap ends (unless wicket)
        const newLegalInOver = updated.filter(
          (d) =>
            d.over_no === data.over_no &&
            (d.extras_type === null || d.extras_type === 3 || d.extras_type === 4)
        ).length
      ...
      {fixtureId && (
        <>
                    ...
                    <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                      {ov.balls.map((b, i) => (
                        <span
                          key={i}
                          style={{
                            background: b.dismissed_batter_id
                              ? 'var(--hotpink)'
                              : b.extras_type === 2
                                ? 'var(--surface-alt)'
                                : b.extras_type === 1
          ...
          >
            ...
            <button
              onClick={submitBall}
              disabled={saving || !striker || !bowler}
